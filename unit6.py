from PIL import Image, ImageDraw

first = (
    146, 399, 163, 403, 170, 393, 169, 391, 166, 386, 170, 381, 170, 371, 170, 355, 169, 346, 167, 335, 170, 329, 170, 320, 170, 310, 171, 301, 173, 290, 178, 289, 182, 287, 188, 286, 190, 286, 192, 291, 194, 296, 195, 305, 194, 307, 191, 312, 190, 316, 190, 321, 192, 331, 193, 338, 196, 341, 197, 346, 199, 352, 198, 360, 197, 366, 197, 373, 196, 380, 197, 383, 196, 387, 192, 389, 191, 392, 190, 396, 189, 400, 194, 401, 201, 402, 208, 403, 213, 402, 216, 401, 219, 397, 219, 393, 216, 390, 215, 385, 215, 379, 213, 373, 213, 365, 212, 360, 210, 353, 210, 347, 212, 338, 213, 329, 214, 319, 215, 311, 215, 306, 216, 296, 218, 290, 221, 283, 225, 282, 233, 284, 238, 287, 243, 290, 250, 291, 255, 294, 261, 293, 265, 291, 271, 291, 273, 289, 278, 287, 279, 285, 281, 280, 284, 278, 284, 276, 287, 277, 289, 283, 291, 286, 294, 291, 296, 295, 299, 300, 301, 304, 304, 320, 305, 327, 306, 332, 307, 341, 306, 349, 303, 354, 301, 364, 301, 371, 297, 375, 292, 384, 291, 386, 302, 393, 324, 391, 333, 387, 328, 375, 329, 367, 329, 353, 330, 341, 331, 328, 336, 319, 338, 310, 341, 304, 341, 285, 341, 278, 343, 269, 344, 262, 346, 259, 346, 251, 349, 259, 349, 264, 349, 273, 349, 280, 349, 288, 349, 295, 349, 298, 354, 293, 356, 286, 354, 279, 352, 268, 352, 257, 351, 249, 350, 234, 351, 211, 352, 197, 354, 185, 353, 171, 351, 154, 348, 147, 342, 137, 339, 132, 330, 122, 327, 120, 314, 116, 304, 117, 293, 118, 284, 118, 281, 122, 275, 128, 265, 129, 257, 131, 244, 133, 239, 134, 228, 136, 221, 137, 214, 138, 209, 135, 201, 132, 192, 130, 184, 131, 156, 141, 165, 135, 169, 131, 176, 130, 187, 134, 191, 140, 191, 146, 186, 150, 179, 155, 175, 157, 168, 157, 163, 157, 159, 157, 158, 164, 159, 175, 159, 181, 157, 191, 154, 197, 153, 205, 153, 210, 152, 212, 147, 215, 146, 218, 143, 220, 132, 220, 125, 217, 119, 209, 116, 196, 115, 185, 114, 172, 114, 167, 112, 161, 109, 165, 107, 170, 99, 171, 97, 167, 89, 164, 81, 162, 77, 155, 81, 148, 87, 140, 96, 138, 105, 141, 110, 136, 111, 126, 113, 129, 118, 117, 128, 114, 137, 115, 146, 114, 155, 115, 158, 121, 157, 128, 156, 134, 157, 136, 156, 136,175, 129, 170, 131, 159
)
second = (
    134, 157, 134, 160, 130, 170, 125, 176, 114, 176, 102, 173, 103, 172, 108, 171, 111, 163, 115, 156, 116, 149, 117, 142, 116, 136, 115, 129, 115, 124, 115, 120, 115, 115, 117, 113, 120, 109, 122, 102, 122, 100, 121, 95, 121, 89, 115, 87, 110, 82, 109, 84, 118, 89, 123, 93, 129, 100, 130, 108, 132, 110, 133, 110, 136, 107, 138, 105, 140, 95, 138, 86, 141, 79, 149, 77, 155, 81, 162, 90, 165, 97, 167, 99, 171, 109, 171, 107, 161, 111, 156, 113, 170, 115, 185, 118, 208, 117, 223, 121, 239, 128, 251, 133, 259, 136, 266, 139, 276, 143, 290, 148, 310, 151, 332, 155, 348, 156, 353, 153, 366, 149, 379, 147, 394, 146, 399
)
pixels = first + second 
img_size = (480, 480)


## noob solution 😭
img = Image.new('RGB', img_size)
imgObj = img.load()
for index in range(0,len(pixels),2):
    imgObj[pixels[index], pixels[index+1]] = (255,255,255)

img.save('./resources/noob_cow.png')


## pro solution 😍😁👌
#(disclaimer - i did this completely by myself)
img = Image.new('RGB', img_size, color='black')

#mask image for the gradient fill
mask = Image.new('L', img_size, 0)
draw_mask = ImageDraw.Draw(mask)

#group pixels
points = [(pixels[i], pixels[i + 1]) for i in range(0, len(pixels), 2)]

draw_mask.polygon(points, fill=255)

#create a gradient fill using the mask
gradient = Image.new('RGB', img_size)
for y in range(img_size[1]):
    for x in range(img_size[0]):
        gradient.putpixel((x, y), (int(255 * (x / img_size[0])), int(255 * (y / img_size[1])), 255))

#apply the gradient on the original image
img = Image.composite(gradient, img, mask)
draw = ImageDraw.Draw(img)

#draw a white outline and save
draw.line(points + [points[0]], fill='white', width=3)
img.save('./resources/pro_cow.png')



## sigit chief master of the devops solution 😱😎😎🙏
#(disclaimer - chatgpt might've helped a lil with this one because im still sigitdogs and not sigitchief😴🤐)
import math

#number of frames for the gif
num_frames = 72  #5 degrees rotation each frame
frame_duration_ms = 20
frames = []

center_x, center_y = img_size[0] // 2, img_size[1] // 2

for i in range(num_frames):#create a new image for each frame
    
    frame = Image.new('RGB', img_size, color='black')
    draw_frame = ImageDraw.Draw(frame)
    
    #calc the angle of rotation
    angle = (360 / num_frames) * i
    rotated_points = []

    for x, y in points:
        temp_x = x - center_x
        temp_y = y - center_y
        
        # Rotate
        rotated_x = temp_x * math.cos(math.radians(angle)) - temp_y * math.sin(math.radians(angle))
        rotated_y = temp_x * math.sin(math.radians(angle)) + temp_y * math.cos(math.radians(angle))
        
        # Save the calculated position
        rotated_points.append((rotated_x + center_x, rotated_y + center_y))
    
    #mask image for the gradient fill
    mask = Image.new('L', img_size, 0)
    draw_mask = ImageDraw.Draw(mask)
    draw_mask.polygon(rotated_points, fill=255)
    
    #create a gradient fill using the mask
    gradient = Image.new('RGB', img_size)
    for y in range(img_size[1]):
        for x in range(img_size[0]):
            gradient.putpixel((x, y), (int(255 * (x / img_size[0])), int(255 * (y / img_size[1])), 255))
    
    #apply the gradient on the original image
    filled_shape = Image.composite(gradient, frame, mask)
    draw_frame = ImageDraw.Draw(filled_shape)
    
    #draw a white outline and save the frame
    draw_frame.line(rotated_points + [rotated_points[0]], fill='white', width=1) 
    frames.append(filled_shape)

#save all frames as a gif
frames[0].save('./resources/spinning_cow - devops_master😱😎.gif', save_all=True, append_images=frames[1:], duration=frame_duration_ms, loop=0)
